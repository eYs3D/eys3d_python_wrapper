...
def test_config():
    ...
    dev = pipe.get_device()
    conf.set_preset_mode_config(0x162, mode, dev.get_usb_type())
    pipe.start(conf)
    import time
    time.sleep(1)
...
def test_ModeConfig():  # To test ModeConfig
    modeConfig = eys3d.ModeConfig(0x162, mode, 3)
...
def test_write_read_sensor_register():
    ...
    sensor_addr = 0x305E #8062
    sensor_n_id = 0x30 # 8062
    ...
    register.set_sensor_register(sensor_addr, (sensor_value - 1), sensor_type,
                      sensor_n_id)
    sensor_value2 = byteswap(register.get_sensor_register(sensor_addr, sensor_type,
                                    sensor_n_id))
...
def test_accuracy(ratio, dist):
    ...
    if snapshot:
        time.sleep(1) # Wait for accuracy calculated
    dret, dframe = pipe.wait_depth_frame(3300)
    info = dframe.get_depth_accuracy_info()
